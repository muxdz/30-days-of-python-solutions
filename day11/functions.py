...
show_args(name="Alice", age=25, city="New York")
...
def same_data_type(list):
    return all(type(element) == type(list[0]) for element in list)
...
print(is_valid('my_var'))
print(is_valid('2my_var'))
...
def most_spoken_languages(countries):
    ...
    top_ten_languages = sorted(language_counts.items(), key=lambda item: item[1], reverse=True)[:10]
    return top_ten_languages

print(most_spoken_languages(countries))

def most_populated_countries(countries):
    top_ten_countries = sorted(countries, key=lambda country: country["population"], reverse=True)[:10]
